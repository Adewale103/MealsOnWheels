...
const ManageOrder = () => {
  ...
  const sum = (id, price) => {
    const sumation = price * quantity(id) * 500;
    total += sumation;
    return sumation;
  };
  ...
  const config = {
    ...
    email: "ade@gmail.com",
    amount: 1150000,
    publicKey: "pk_test_60ab90ad7d5b93d253008ce9f5004c05e07f67c0",
  };
      ...
      {product.length ? (
        <div className="lg:flex lg:flex-row md:flex-row  flex flex-col md:flex lg:w-5/6 md:w-5/6 w-full mx-auto  justify-around">
          <div className="lg:w-1/2 md:w-1/2 w-full lg:order-1 md:order-1 order-2 flex lg:justify-start  justify-center items-center">
            <div className="p-4">
              <h1 className="lg:text-4xl md:text-4xl text-2xl text-dark font-bold py-4">
                Edit Delivery Details
              </h1>
              <form className="py-4">
                <input
                  type="email"
                  className="border-2 border-lightgray px-4 py-1 rounded w-full outline-offset-1 outline-primary focus:border-none mb-4"
                ...
                <br />
                <input
                  className="bg-primary text-white  px-4 py-1 rounded w-full font-bold cursor-pointer "
                  type="submit"
                  value="Submit"
                />
              </form>
          ...
          <div className="lg:w-1/2 md:w-1/2 lg:order-2 md:order-2 order-1 w-full flex justify-center">
            <div className="p-4 mt-4 w-full">
              <h1 className="text-3xl  text-dark">
                Form{" "}
                <span className="font-bold">Meals On Wheels Food Delivery</span>
              </h1>
              <h2 className="text-2xl   text-dark">Arriving in 20-30 min</h2>
              <h2 className="text-2xl  text-dark">
                312, Herbert Macaulay Street, Sabo, Yaba
              </h2>
              <br />
              <br />
              {product.map((pd) => {
                return (
                  <div className="flex justify-between rounded items-center m-lg shadow-lg p-4">
                    ...
                    <div className="text-center">
                      <button
                        onClick={() => increment(pd._id)}
                        className="bg-primary px-4 py-2 rounded-full leading-none"
                      >
                        +
                      </button>
              ...
              <br />
              <br />
              <p className="flex justify-between text-lg text-dark">
                <span>Sub total:</span> <span>₦{Math.round(total)}</span>
              </p>
              <p className="flex justify-between text-lg text-dark">
                <span>Tex: 5%</span> <span>₦{Math.round(total * 0.05)}</span>
              </p>
              <p className="flex justify-between text-2xl font-bold">
                <span>Total:</span>{" "}
                <span>₦{Math.round(Math.round(total * 0.05) + total)}</span>
              </p>
              <form onSubmit={handleSubmit}>
                <button
                  type="submit"
                  className="bg-lightgray mt-4 text-white bg-primary  px-4 py-1 rounded w-full font-bold cursor-pointer "
                >
                  MakePayment
                </button>
              </form>
